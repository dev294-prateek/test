...
class Teacher_model extends CI_Model
{
    ...
    {   $data = array('section' => $_SESSION['section_id']);
        return $this->db->get_where('student',$data)->num_rows();
    }
    public function all_class_g_count()
    {    $data = array('section_id' => $_SESSION['section_id']);
        return $this->db->get_where('class_gallery',$data)->num_rows();
    }
    public function all_p_count()
    {
        $data=array('teacher_id'=>$_SESSION['teacher_id'],'status'=>1,'running_year'=>$_SESSION['running_year']);
        return $this->db->get_where('period_allotment',$data)->num_rows();
    }
    function lesson_plan_count(){
        $data=array('teacher_id'=>$_SESSION['teacher_id']);
        $work_data = $this->db->get_where('lesson_plan',$data)->num_rows();
        return $work_data;
    }
    function lesson_plan_p_count(){
        $data=array('teacher_id'=>$_SESSION['teacher_id'],'approve_status'=>0);
        $work_data = $this->db->get_where('lesson_plan',$data)->num_rows();
        return $work_data;
    }
    function lesson_plan_a_count(){
        $data=array('teacher_id'=>$_SESSION['teacher_id'],'approve_status'=>1);
        $work_data = $this->db->get_where('lesson_plan',$data)->num_rows();
        return $work_data;
    }
    /*==================================================================*/
    ...
    function add_student($data)
    {
       ...
       /* unset($data['section']);*/
        ...
        $this->teacher_model->add_enroll( $data2);
        if ($this->db->affected_rows() > 0)
        { return TRUE;} else {return FALSE;}
    }
  ...
  /*      unset($data['section']);*/
        ...
        $x=$this->teacher_model->update_enroll($data2);
        if ($this->db->affected_rows() > 0)
        { return TRUE;} else {return FALSE;}
    }
    ...
    public function list_all_student()
    {
        ...
        foreach ($x as $z)
        {
            $y=$this->teacher_model->list_enroll_by_student_id($x[$temp]['student_id']);
    ...
    public function list_student_by_id($id)
    {
        ...
        $y=$this->teacher_model->list_enroll_by_student_id($id);
        $x['class']=$y['class_id'];
    ...
    function teacher_name($id){
        $data=array('teacher_id'=>$id);
        $x=$this->db->get_where('teacher',$data)->row_array();
        $y=$this->teacher_model->employee_name($x['employee_id']);
        return $y;
    }
    ...
    /*==================================================================*/
    function class_work(){
        $data=array('status'=>1,'teacher_id'=>$_SESSION['teacher_id']);
    ...
    function list_class_gallery_by_teacher_id_active($id){
        $data=array('running_year'=>$_SESSION['running_year'],'teacher_id'=>$id,'status'=>1);
        $gal = $this->db->get_where('class_gallery',$data)->result_array();
        return $gal;
    }
